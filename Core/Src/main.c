...
int main(void)
{
  ...
  if (HAL_I2C_IsDeviceReady(&hi2c1, ST7032_ADDR, 1, 100) != HAL_OK) {
      const char *err_msg = "ERROR: ST7032 not responding on I2C\r\n";
...
/* USER CODE BEGIN 4 */
// Your additional helper functions go here
void AdcTask(void *argument) {
  ...
  for (;;) {
    ...
    adc_val_0 = HAL_ADC_GetValue(&hadc1);
    ...
    snprintf(msg, sizeof(msg), "ADC0: %lu, ADC1: %lu\r\n", adc_val_0, adc_val_1);
...
{
  /* USER CODE BEGIN 6 */
  // Optional: log or blink LED here
  /* USER CODE END 6 */
}
